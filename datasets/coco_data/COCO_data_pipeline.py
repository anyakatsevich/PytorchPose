...
class Cocokeypoints(Dataset):
    ...
    def __getitem__(self, index):
        try:
            return self._get_item_full(index)
        except Exception as e:
            print(e)
            return ['error']*5
            

    def _get_item_full(self, index):
...
#        print img.shape
        if "COCO_val" in self.data[idx]['dataset']:
            mask_filepath = self.mask_dir + 'val2014/mask_COCO_val2014_' + img_idx + 'jpg'
            mask_miss = cv2.imread(mask_filepath, 0)
        elif "COCO" in self.data[idx]['dataset']:
            mask_filepath = self.mask_dir + 'train2014/mask_COCO_train2014_' + img_idx + 'jpg'
            mask_miss = cv2.imread(mask_filepath, 0)
#        print self.root + 'mask2014/val2014_mask_miss_' + img_idx + 'png'
        ...
        for item in [img, heatmaps, heat_mask, pafs, paf_mask]:
            if type(item) == None:
                raise Exception("Found item of type None")
